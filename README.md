...
| NVENC D3D12 | `D3DVIDEOENCODER_ENABLE_NVENC=ON` 時に制約付き対応。NV12/P010入力のみ |
...
`NvencD3D12` はD3D12側の初期backendとして追加しています。ただし、現段階では以下の制約があります。

```text
- inputFormat は internalFormat と同じ必要がある
- 対応入力は NV12 / P010
- RGB -> NV12/P010 の D3D12Processing 接続は次段階
- 入力 resource state は D3D12_RESOURCE_STATE_COMMON を要求
- 出力は .h264 / .h265 の elementary stream
```

つまり、現在の `NvencD3D12` は「D3D12 NV12/P010 resourceを直接NVENCへ渡す最小経路」です。
...
`D3DVIDEOENCODER_ENABLE_NVENC=OFF` の場合、`NvencD3D12` は未有効backendとして `D3DVideoEncoderError` を投げます。`MediaFoundation` と native `D3D12VideoEncode` は引き続き未対応です。
