...
void upload_nv12_frame(
    ...
    for (uint32_t y = 0; y < height; ++y) {
        ...
        for (uint32_t x = 0; x < width; ++x) {
            row[x] = static_cast<uint8_t>(48 + ((x + y + frameIndex * 9) & 0x3f));
        }
    }
...
int main() {
    ...
    std::cout << "D3D12 Video Encode backend is not enabled; skipping async smoke test.\n";
    return 0;
#else
    try {
        ...
        std::cout << "D3D12VideoEncode async H264/NV12 supported=" << cap.supported
                  << " message=" << cap.message << "\n";
        if (!cap.supported) {
        ...
        for (uint32_t i = 0; i < frames; ++i) {
            ...
            throw_if_failed(device->CreateCommittedResource(
                ...
                nullptr,
                IID_PPV_ARGS(&texture)), "Create NV12 texture");
            upload_nv12_frame(*core, texture.Get(), width, height, i);
            textures.push_back(std::move(texture));
        }

        const std::filesystem::path outPath = "d3d12video_h264_nv12_async_smoke.mp4";
        ...
        std::cout << "D3D12 Video Encode async MP4 output size: " << fileSize << " bytes\n";
        if (fileSize <= 1024) {
            std::cerr << "FAILED: D3D12 Video Encode async MP4 output is missing or too small.\n";
            return 1;
        }
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "D3D12 Video Encode H.264/NV12 async smoke failed: " << e.what() << "\n";
        return 1;
    }
#endif
}
