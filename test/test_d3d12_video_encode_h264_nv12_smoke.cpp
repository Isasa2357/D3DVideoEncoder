...
int main() {
    ...
    std::cout << "D3D12 Video Encode backend is not enabled; skipping smoke test.\n";
    return 0;
#else
    try {
        ...
        const std::filesystem::path outPath = "d3d12video_h264_nv12_smoke.h264";
        ...
        std::cout << "D3D12 Video Encode output size: " << fileSize << " bytes\n";
        if (fileSize == 0) {
            std::cerr << "FAILED: D3D12 Video Encode output is empty.\n";
            return 1;
        }
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "D3D12 Video Encode H.264/NV12 smoke failed: " << e.what() << "\n";
        return 1;
    }
#endif
}
