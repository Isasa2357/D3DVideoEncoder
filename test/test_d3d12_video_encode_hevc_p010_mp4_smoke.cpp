...
void upload_p010_frame(
    ...
    for (uint32_t y = 0; y < height; ++y) {
        ...
        for (uint32_t x = 0; x < width; ++x) {
            const uint16_t y10 = static_cast<uint16_t>(64u + ((x * 3u + y * 2u + frameIndex * 11u) % 256u));
...
bool is_parameter_set_mux_error(const std::string& message) {
    return message.find("VPS/SPS/PPS") != std::string::npos ||
           message.find("parameter sets") != std::string::npos ||
           message.find("hvcC") != std::string::npos;
}
...
int main() {
    ...
    std::cout << "D3D12 Video Encode backend is not enabled; skipping HEVC/P010 MP4 smoke test.\n";
    return 0;
#else
    try {
        ...
        std::cout << "D3D12VideoEncode HEVC/P010 supported=" << cap.supported
                  << " message=" << cap.message << "\n";
        if (!cap.supported) {
            std::cout << "Native D3D12 Video Encode HEVC/P010 is not supported on this device; skipping.\n";
            return 0;
        }
        ...
        throw_if_failed(device->CreateCommittedResource(
            ...
            nullptr,
            IID_PPV_ARGS(&texture)), "Create P010 texture");

        const std::filesystem::path outPath = "d3d12video_hevc_p010_smoke.mp4";
        ...
        for (uint32_t i = 0; i < 16; ++i) {
            upload_p010_frame(*core, texture.Get(), width, height, i);
            encoder.write(texture.Get(), D3D12_RESOURCE_STATE_COPY_DEST);
        }
        encoder.close();

        const auto fileSize = std::filesystem::file_size(outPath);
        std::cout << "D3D12 Video Encode HEVC/P010 MP4 output size: " << fileSize << " bytes\n";
        if (fileSize <= 1024) {
            std::cerr << "FAILED: D3D12 Video Encode HEVC/P010 MP4 output is too small.\n";
            return 1;
        }
        return 0;
    } catch (const D3DVideoEncoderError& e) {
        const std::string msg = e.what();
        if (is_parameter_set_mux_error(msg)) {
            std::cout << "SKIP: D3D12 Video Encode HEVC/P010 worked, but this driver did not emit VPS/SPS/PPS needed for MP4/MKV mux: " << msg << "\n";
            return 0;
        }
        std::cout << "SKIP: D3D12 Video Encode HEVC/P010 MP4 path is not available or rejected by the driver: " << msg << "\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "D3D12 Video Encode HEVC/P010 MP4 smoke failed: " << e.what() << "\n";
        return 1;
    }
#endif
}
