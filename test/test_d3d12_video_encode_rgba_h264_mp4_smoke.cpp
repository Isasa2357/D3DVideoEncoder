...
void upload_rgba_pattern(
    ...
    for (uint32_t y = 0; y < height; ++y) {
        ...
        for (uint32_t x = 0; x < width; ++x) {
            ...
            row[p + 0] = static_cast<uint8_t>((x * 255u) / std::max<uint32_t>(width - 1, 1));
            row[p + 1] = static_cast<uint8_t>((y * 255u) / std::max<uint32_t>(height - 1, 1));
            row[p + 2] = 96;
            row[p + 3] = 255;
        }
    }
...
int main() {
    ...
    std::cout << "D3D12 Video Encode backend is not enabled; skipping RGBA smoke test.\n";
    return 0;
#else
    try {
        ...
        std::cout << "D3D12VideoEncode H264/NV12 supported=" << cap.supported
                  << " message=" << cap.message << "\n";
        if (!cap.supported) {
        ...
        throw_if_failed(device->CreateCommittedResource(
            ...
            nullptr,
            IID_PPV_ARGS(&texture)), "Create RGBA texture");

        upload_rgba_pattern(*core, texture.Get(), width, height);

        const std::filesystem::path outPath = "d3d12video_rgba_h264_mp4_smoke.mp4";
        ...
        std::cout << "D3D12 Video Encode RGBA->H264 MP4 output size: " << fileSize << " bytes\n";
        if (fileSize == 0) {
            std::cerr << "FAILED: D3D12 Video Encode RGBA MP4 output is empty.\n";
            return 1;
        }
        return 0;
    } catch (const std::exception& e) {
        const std::string message = e.what();
        std::cerr << "D3D12 Video Encode RGBA/H.264 MP4 smoke failed: " << message << "\n";
        if (message.find("SPS") != std::string::npos || message.find("PPS") != std::string::npos || message.find("avcC") != std::string::npos) {
            std::cout << "Driver did not expose SPS/PPS in the native D3D12 Video Encode bitstream; skipping MP4 smoke.\n";
            return 0;
        }
        return 1;
    }
