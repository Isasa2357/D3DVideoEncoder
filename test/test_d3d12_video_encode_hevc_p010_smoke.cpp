...
void upload_p010_frame(
    ...
    for (uint32_t y = 0; y < height; ++y) {
        ...
        for (uint32_t x = 0; x < width; ++x) {
            const uint16_t y10 = static_cast<uint16_t>(64u + ((x + y + frameIndex * 7u) % 256u));
...
int main() {
    ...
    std::cout << "D3D12 Video Encode backend is not enabled; skipping HEVC/P010 smoke test.\n";
    return 0;
#else
    try {
        ...
        const std::filesystem::path outPath = "d3d12video_hevc_p010_smoke.h265";
        ...
        std::cout << "D3D12 Video Encode HEVC/P010 output size: " << fileSize << " bytes\n";
        if (fileSize == 0) {
            std::cerr << "FAILED: D3D12 Video Encode HEVC/P010 output is empty.\n";
            return 1;
        }
        return 0;
    } catch (const D3DVideoEncoderError& e) {
        std::cout << "SKIP: D3D12 Video Encode HEVC/P010 path is not available or rejected by the driver: " << e.what() << "\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "D3D12 Video Encode HEVC/P010 smoke failed: " << e.what() << "\n";
        return 1;
    }
#endif
}
