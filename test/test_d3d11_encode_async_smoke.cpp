...
int main() {
    try {
        ...
        const std::filesystem::path outputPath = std::filesystem::absolute("d3d11_encode_async_smoke.mp4");
        std::error_code ec;
        ...
        std::cout << "D3D11EncodeAsyncSmoke wrote and read back " << info.sampleCount
                  << " video samples from " << outputPath.string() << "\n";
        return 0;
    } catch (const std::exception& e) {
        std::cerr << "D3D11 async encode smoke test failed: " << e.what() << "\n";
        return 1;
    }
}
